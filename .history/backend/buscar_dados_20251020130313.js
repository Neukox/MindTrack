...
async function buscarDados() {
  try {
    ...
    if (reflexoes.length === 0) {
      ...
      reflexoes.forEach((reflection, index) => {
        console.log(`${index + 1}. PUT http://localhost:3000/edicao-registros/reflexao/${reflection.id}/usuario/cmgwakeho0000weu8r3vl9r3q`);
      });
      
      console.log('\n�️ URLs para testes de exclusão individual:');
      reflexoes.forEach((reflection, index) => {
        console.log(`${index + 1}. DELETE http://localhost:3000/edicao-registros/reflexao/${reflection.id}/usuario/cmgwakeho0000weu8r3vl9r3q`);
      });
      
      if (reflexoes.length >= 2) {
        console.log('\n📦 JSON para teste em lote (2 reflexões):');
        console.log('URL: PUT http://localhost:3000/edicao-registros/reflexoes/usuario/cmgwakeho0000weu8r3vl9r3q');
      ...
      if (reflexoes.length >= 3) {
        ...
        console.log('URL: DELETE http://localhost:3000/edicao-registros/reflexoes/usuario/cmgwakeho0000weu8r3vl9r3q');
        console.log(JSON.stringify({
