import { Controller, Body, Post, Get, Query, Param, UseGuards, Request, BadRequestException } from '@nestjs/common';
...
export class ReflexaoController {
  ...
  @ApiOperation({
    ...
    description: 'Retorna as reflexões do usuário autenticado baseado no token JWT.',
  })
  @ApiResponse({
    status: 200,
  ...
  async getCategoriasEstatisticas(@Request() req: any) {
    ...
    const categorias = await this.reflexaoService.getCategoriasEstatisticas(userId);
    return categorias;
  }
