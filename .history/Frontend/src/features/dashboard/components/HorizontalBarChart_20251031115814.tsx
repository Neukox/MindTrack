    ...
    const carregarDados = async () => {
      try {
        ...
        const dadosFormatados = Object.entries(response.data).map(
          ([emocao, dados]) => ({
            nome: emocao,
            valor: dados.count, // Corrigido: usar count em vez de total
          })
        );
      ...
      <ResponsiveContainer>
        ...
        >
          ...
          <Tooltip
            ...
            // Mostrar a quantidade exata ao passar o mouse e o rótulo (nome) separadamente
            formatter={(value: number) => `${Math.round(value)} registros`}
