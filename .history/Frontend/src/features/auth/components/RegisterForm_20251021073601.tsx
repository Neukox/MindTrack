...
export default function TelaCadastro() {
  ...
  function onSubmit(data: FormData) {
    ...
    // sucesso local
    toast.success(`Conta criada: ${data.nome}`)
    navigate('/')
  }
...
<>
 ...
 <Helmet>
  ...
  {errors.senha && <p className="text-sm text-red-500 mb-4">Senha inválida (mínimo 8 caracteres)</p>}
  ...
  {...register('confirmarSenha', { required: true, validate: value => value === watch('senha') || 'As senhas não coincidem' })}
  placeholder="********"
  className="w-full pr-10 px-4 py-2 border border-slate-200 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300"
  />
        <button
  ...
  {errors.confirmarSenha && <p className="text-sm text-red-500 mb-6">{String(errors.confirmarSenha.message || 'Confirme a senha')}</p>}

        {/* Link que envia o formulário */}
        <button
          type="submit"
