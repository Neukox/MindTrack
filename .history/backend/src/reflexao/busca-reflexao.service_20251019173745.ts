...
export class BuscaReflexaoService {
  ...
  async getReflexoesByEmotion(userId: string, emotion: string): Promise<any[]> {
    try {
      if (!userId || !emotion) {
        throw new BadRequestException(
          'ID do usuário e emoção são obrigatórios.',
        );
      }
      ...
      const reflexoes = await this.prisma.reflection.findMany({
        where: {
          userId,
          // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
          emotion: emotion as any, // Converte para enum Emotion
        },
        orderBy: { createdAt: 'desc' },
      });

      console.log('Reflexões encontradas por emoção:', reflexoes?.length || 0);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-return
      return reflexoes;
    } catch (error) {
      if (error instanceof BadRequestException) {
      ...
      console.error('Erro ao buscar reflexões por emoção:', error);
      throw new BadRequestException(
        'Erro interno. Tente novamente mais tarde.',
      );
    }
  ...
  async getReflexoesByDateRange(
    ...
    try {
      if (!userId || !startDate || !endDate) {
        throw new BadRequestException(
          'ID do usuário, data inicial e data final são obrigatórios.',
        );
      }
      ...
      console.log('Buscando reflexões entre:', start, 'e', end);

      // Busca reflexões em um período específico
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
      const reflexoes = await this.prisma.reflection.findMany({
        where: {
          userId,
      ...
      console.log('Reflexões encontradas no período:', reflexoes?.length || 0);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-return
      return reflexoes;
    } catch (error) {
      if (error instanceof BadRequestException) {
      ...
      console.error('Erro ao buscar reflexões por período:', error);
      throw new BadRequestException(
        'Erro interno. Tente novamente mais tarde.',
      );
    }
  ...
  async getReflexoesWithFilters(
    ...
    try {
      ...
      // Constrói dinamicamente o objeto where baseado nos filtros
      ...
      if (filters.emotion) {
        whereConditions.emotion = filters.emotion as any;
      }
      ...
      // Busca reflexões com filtros combinados
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
      const reflexoes = await this.prisma.reflection.findMany({
      ...
      console.log('Reflexões encontradas com filtros:', reflexoes?.length || 0);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-return
      return reflexoes;
    } catch (error) {
      if (error instanceof BadRequestException) {
      ...
      console.error('Erro ao buscar reflexões com filtros:', error);
      throw new BadRequestException(
        'Erro interno. Tente novamente mais tarde.',
      );
    }
