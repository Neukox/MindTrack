...
export function DashboardPage() {
  ...
  const carregarDados = async () => {
    try {
      ...
      if (dadosEstatisticasSemana.success) {
        setRegistrosSemana(dadosEstatisticasSemana.data.registrosEssaSemana || 0);
        setCrescimentoSemana(dadosEstatisticasSemana.data.crescimentoPercentual || 0);
      }

      if (dadosDiasConsecutivos.success) {
        const dias = dadosDiasConsecutivos.data.diasConsecutivos;
  ...
  const formatarPercentual = (valor: number) => {
    ...
    const cor = valor > 0 ? "text-green-500" : valor < 0 ? "text-red-500" : "text-gray-500";
    return (
      <span className={cor}>
