...
export class BuscaReflexaoService {
  ...
  async getReflexaoById(id: string, userId: string): Promise<any> {
    try {
      if (!id || !userId) {
        throw new BadRequestException('ID da reflexão e do usuário são obrigatórios.');
      }
      ...
      if (!reflexao) {
        throw new BadRequestException('Reflexão não encontrada ou não pertence ao usuário.');
      }
  ...
  async getReflexoesByCategory(userId: string, category: string): Promise<any[]> {
    try {
      if (!userId || !category) {
        throw new BadRequestException('ID do usuário e categoria são obrigatórios.');
      }
