...
export class DiasConsecutivosService {
  ...
  async calcularDiasConsecutivos(userId: string): Promise<{
    ...
    try {
      ...
      const diasConsecutivos =
        this.calcularSequenciaConsecutiva(diasComReflexoes);
      ...
      const diferencaDias = Math.floor(
        (hoje.getTime() - ultimoDiaComReflexao.getTime()) /
          (1000 * 60 * 60 * 24),
      );
  ...
  private calcularSequenciaConsecutiva(dias: Date[]): number {
    ...
    const ultimoDiaComReflexao = new Date(dias[0]);
    ultimoDiaComReflexao.setHours(0, 0, 0, 0);

    // Calcula diferença entre hoje e último dia com reflexão
    const diferencaParaHoje = Math.floor(
      (hoje.getTime() - ultimoDiaComReflexao.getTime()) / (1000 * 60 * 60 * 24),
    );

    // Se a última reflexão foi há mais de 1 dia, a sequência está quebrada
    ...
    // Se só tem um dia com reflexão, retorna 1
    if (dias.length === 1) return diasConsecutivos;
  ...
  async obterEstatisticasDetalhadas(userId: string): Promise<{
    ...
    try {
      ...
      const { data: diasConsecutivosData } =
        await this.calcularDiasConsecutivos(userId);
      ...
      const maiorSequencia =
        this.calcularMaiorSequenciaHistorica(diasComReflexoes);

      // Calcula percentual de dias ativos (últimos 30 dias)
      const percentualDiasAtivos =
        this.calcularPercentualDiasAtivos(diasComReflexoes);
  ...
  private calcularPercentualDiasAtivos(diasComReflexoes: Date[]): number {
    ...
    const diasNoPeriodo = diasComReflexoes.filter(
      (dia) => dia >= trintaDiasAtras,
    );
   ...
   */
  private gerarUltimosDias(
    reflexoes: { createdAt: Date }[],
    numeroDias: number,
  ): Array<{
    data: string;
    temReflexao: boolean;
