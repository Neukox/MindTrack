import { Controller, Get, Req, UseGuards, HttpStatus, HttpException } from '@nestjs/common';
...
export class RegistrosEssaSemanaController {
  ...
  async getRegistrosEssaSemana(@Req() req: AuthenticatedRequest) {
    try {
      ...
      const result = await this.registrosEssaSemanaService.getRegistrosEssaSemana(userId);
  ...
  async getEstatisticasEssaSemana(@Req() req: AuthenticatedRequest) {
    try {
      const userId = req.user.sub;
      const result = await this.registrosEssaSemanaService.getEstatisticasEssaSemana(userId);
