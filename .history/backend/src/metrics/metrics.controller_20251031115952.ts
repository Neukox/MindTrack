...
export class MetricsController {
  ...
  @ApiOperation({
    summary: 'Obter última reflexão criada pelo usuário',
    description:
      'Retorna informações sobre a última reflexão criada pelo usuário.',
  })
  @ApiBearerAuth()
  @ApiResponse({
    status: 200,
  ...
  @Get('last-reflection')
  async getLastReflection(@User('sub') userId: string) {
    const lastReflection =
      await this.reflexaoService.getLastReflectionCreatedByUser(userId);
    ...
    if (!lastReflection) {
      return {
        ...
        data: {
          ...
          tempoDesdeUltimaReflexao: 'Nenhuma reflexão encontrada',
          reflexaoMaisRecente: null,
        },
      };
    }

    // Calcular tempo desde a última reflexão
    const agora = new Date();
    const criadaEm = new Date(lastReflection.createdAt);
    const diffMs = agora.getTime() - criadaEm.getTime();
    
    const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));
    const diffHours = Math.floor((diffMs % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    const diffMinutes = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60));

    let tempoDesdeUltimaReflexao: string;
    if (diffDays > 0) {
      tempoDesdeUltimaReflexao = `${diffDays} dia${diffDays > 1 ? 's' : ''} atrás`;
    } else if (diffHours > 0) {
      tempoDesdeUltimaReflexao = `${diffHours} hora${diffHours > 1 ? 's' : ''} atrás`;
    } else if (diffMinutes > 0) {
      tempoDesdeUltimaReflexao = `${diffMinutes} minuto${diffMinutes > 1 ? 's' : ''} atrás`;
    } else {
      tempoDesdeUltimaReflexao = 'Agora mesmo';
    }

    return {
      success: true,
      data: {
        dataUltimaReflexao: lastReflection.createdAt.toISOString(),
        tempoDesdeUltimaReflexao,
        reflexaoMaisRecente: {
          id: lastReflection.id,
          title: lastReflection.title,
          categoria: lastReflection.category,
          emocao: lastReflection.emotion,
          createdAt: lastReflection.createdAt.toISOString(),
        },
      },
    };
  }
