...
export class EdicaoRegistrosService {
  ...
  // Algoritmo para buscar uma reflexão específica por ID
  async getReflexaoById(reflexaoId: string, userId: string): Promise<any> {
    try {
      ...
      console.log('Buscando reflexão:', reflexaoId, 'para usuário:', userId);

      // Busca a reflexão e verifica se pertence ao usuário
      const reflexao = await this.prisma.reflection.findFirst({
        where: {
          id: reflexaoId,
          userId, // Garante que só pode acessar reflexões próprias
        },
        select: {
          id: true,
          title: true,
  ...
  async updateReflexao(
    ...
    try {
      ...
      // Valida e converte enums se fornecidos
      const dadosParaAtualizar: any = {};

      if (updateData.title !== undefined) {
        dadosParaAtualizar.title = updateData.title;
      }

      if (updateData.content !== undefined) {
        dadosParaAtualizar.content = updateData.content;
      }

      if (updateData.category !== undefined) {
        const validCategories = ['ESTUDO', 'PESSOAL', 'ESTAGIO', 'PESQUISA'];
        if (!validCategories.includes(updateData.category as string)) {
          throw new BadRequestException(
            `Categoria inválida. Categorias válidas: ${validCategories.join(', ')}`,
          );
        }
        dadosParaAtualizar.category = updateData.category;
      }
      ...
      if (updateData.emotion !== undefined) {
        ...
        if (!validEmotions.includes(updateData.emotion as string)) {
          throw new BadRequestException(
            `Emoção inválida. Emoções válidas: ${validEmotions.join(', ')}`,
          );
        }
        dadosParaAtualizar.emotion = updateData.emotion;
      }
