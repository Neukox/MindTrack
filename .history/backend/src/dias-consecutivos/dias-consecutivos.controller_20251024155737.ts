...
export class DiasConsecutivosController {
  ...
  async getDiasConsecutivos(@Req() req: AuthenticatedRequest) {
    try {
      ...
      const result = await this.diasConsecutivosService.calcularDiasConsecutivos(userId);
  ...
  async getEstatisticasDetalhadas(@Req() req: AuthenticatedRequest) {
    try {
      const userId = req.user.sub;
      const result = await this.diasConsecutivosService.obterEstatisticasDetalhadas(userId);
