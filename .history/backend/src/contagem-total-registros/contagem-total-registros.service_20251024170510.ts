...
export class ContagemTotalRegistrosService {
  ...
  async contarTotalRegistrosPorUsuario(userId: string) {
    try {
      ...
      const inicioSemestre = new Date(agora.getFullYear(), agora.getMonth() - 5, 1);
      ...
      const crescimentoPercentual = entradasSemestreAnterior === 0 
        ? 100 
        : Math.round(((entradasEsseSemestre - entradasSemestreAnterior) / entradasSemestreAnterior) * 100);
